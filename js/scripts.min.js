...
$(function() {
	...
	afterMove: function(index) {
		...
		if($('.works').hasClass('active')){
			$('.works-link').css('color', '#fff');
		};
		if($('.contacts').hasClass('active')){
			$('.contacts-link').css('color', '#fff');
		};
		if(!$('.works').hasClass('active')){
			$('.works-link').css('color', '#0ff');
		};
		if(!$('.contacts').hasClass('active')){
			$('.contacts-link').css('color', '#f0f');
		};
	...
	$('nav').on('click', function(){
		$('.works-link').css({'animation-name':'pulse1', 'pointer-events':'auto'});
		$('.contacts-link').css({'animation-name':'pulse2', 'pointer-events':'auto'});
	...
	let bluephoto = $('.photo-blue');
	let pinkphoto = $('.photo-pink');
	let wincenter = $(window).width()/2;
	let wincentervertical = $(window).height()/2;
	console.log(wincentervertical);
	document.addEventListener('mousemove', function(e){
		let towincenter = wincenter - e.pageX;
		let towincentervertical = wincentervertical - e.pageY;
		bluephoto.css({'margin-right':(towincenter/25) +"px", 'margin-top':(towincentervertical/25) + 'px'});
		pinkphoto.css({'margin-left':(towincenter/20) +"px", 'margin-top':(-towincentervertical/20) + 'px'});
	});
	$('.works .left').on('click', function(){
		window.open("https://anastasiia34.github.io/Alitmix/", '_blank');
	});
	$('.works .right').on('click', function(){
		window.open("https://anastasiia34.github.io/Fortis/", '_blank');
	});
	...
	$('#someForm').on('submit', function(e) {
		...
		//send to formspree
		$.ajax({
			url:'https://formspree.io/34strekalova@gmail.com',
			method:'POST',
			data:{
				 email:email,
				message:message,
